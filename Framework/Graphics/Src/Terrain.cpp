...
void SpEngine::Graphics::Terrain::Initialize(const std::filesystem::path& fileName, float heightScale)
{
	FILE* file = nullptr;
	fopen_s(&file, fileName.string().c_str(), "rb");
	ASSERT(file != nullptr, "Terrain: file %s was not found", fileName.string().c_str());
...
float SpEngine::Graphics::Terrain::GetHeight(const Math::Vector3& position) const
{
